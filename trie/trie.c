...
/* Trie stores the names as :   a   ---ab  ---abu
                                           ---abh  ---abhi
                                    ---ac  ---ach  ---achu
                                                   ---achu  ---achus
                                b   ---be  ---ben
...
bool check(char *word)
{
    // Set a pointer to the root of the trie
    node *cursor = root;

    // Iterate through every letter
    for (int i = 0; i < strlen(word); i++)
    {
        // Determine the array index corresponding to the letter
        ...
        // Reset cursor, if the letter exists
        cursor = cursor->children[index];

        // Check if we reached the last letter
        if (cursor->is_word)
        {
            return true;
        }
